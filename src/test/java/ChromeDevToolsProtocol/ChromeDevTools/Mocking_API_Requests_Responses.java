...
public class Mocking_API_Requests_Responses {
	 ...
	 *              will only return one record in the UI if you change the author name, to something else.
	 ...
	 * 
	 *      and this will be done- by mocking the request or the response (Whatever works and makes sense !).
	 *
	 * Now, for mocking, we have to:  1- Intercept the call. 2- DO the mocking. For this thee exists:
	 * 
	 * "Fetch Domain"
	...
	public void mockingApi()
	{
		 ...
		 // First I have to enable:
		   //Just I've done earlier with the 'Networking Domain', I have used 'enable', so that:
		 // Network traffic is now flowing from the browser's Chrome DevTools and into our client (Selenium)
		 // This network traffic consists of events. Events are nothing but activities that are triggered. (REMOVE)
		  ...
		  *     Now before I send any request, I can manipulate it.
		...
		if( request.getRequest().getUrl().contains("shetty"))
		{
			...
			//Now, replace the original url with the mocked url:
			 devTools.send(Fetch.continueRequest(request.getRequestId(), mockedUrl, null, null, null, null)); // argument(s) used to mock the data.
			 //18:44 
			 //Continue tomorrow !
		}
		});
